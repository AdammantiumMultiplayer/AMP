...
namespace AMP.Network.Client.NetworkComponents {
    internal class NetworkCreature : NetworkPosition {
        ...
        private void Ragdoll_OnTelekinesisGrabEvent(SpellTelekinesis spellTelekinesis, HandleRagdoll handleRagdoll) {
            if(!IsSending()) {
                creatureNetworkData.RequestOwnership(true);
            }
        }
        
        private void Ragdoll_OnTelekinesisReleaseEvent(SpellTelekinesis spellTelekinesis, HandleRagdoll handleRagdoll, bool lastHandler) {
        ...
        private void Ragdoll_OnUngrabEvent(RagdollHand ragdollHand, HandleRagdoll handleRagdoll, bool lastHandler) {
            if(!lastHandler) return;
