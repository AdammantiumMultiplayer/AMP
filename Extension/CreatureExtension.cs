    ...
    internal static class CreatureExtension {
        ...
        internal static void SmoothDampRagdoll(this Creature creature, Vector3[] vectors, ref Vector3[] velocities, Vector3 pos_offset) {
            ...
            foreach(Ragdoll.Bone bone in creature.ragdoll.bones) {
                ...
                new_vectors[i] = bone.part.transform.position.InterpolateTo(vectors[i] + pos_offset, ref velocities[i], Config.MOVEMENT_DELTA_TIME / 2); i++;
                new_vectors[i] = bone.part.transform.eulerAngles.InterpolateEulerTo(vectors[i], ref velocities[i], Config.MOVEMENT_DELTA_TIME / 2); i++;
            }
